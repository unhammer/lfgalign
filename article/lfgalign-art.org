...
Similarly, on the f-structure level we allow adverbials to remain
 unlinked; adverbials differ from arguments mainly in being
...
If not all arguments of two predicates have LPT-correspondents among
 each other, we have evidence that the predicates themselves are used
 to express different propositions. We do allow linking arguments to
 adverbials though; the monolingual evidence which informed the
 individual grammars may have suggested that a certain participant of
 a verbal situation should be analysed as an argument in one language,
 but as an adverbial in the other -- but in a particular translation,
 they may still correspond semantically.
...
Formally, these are the requirements for linking two f-structure
...
unlinked on the f-structure level. As mentioned, for adjuncts of $p$
we do not even require that they have LPT-correspondence with
arguments/adjuncts of $q$, or vice versa, but \Last[e] ensures that
they are not /linked/ outside of their predicates, which would imply
that $p$ and $q$ did not contain corresponding linked material.
...
set of nodes dominated by $n$ which are word-linked. To link $n_s$ and
$n_t$ (whose projected f-structures must be linked), all nodes in
$LL(n_s)$ must be linked to nodes in $LL(n_t)$. Unlinked nodes
dominated by $n_s$ or $n_t$ do not hinder linking of these nodes.
