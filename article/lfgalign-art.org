#+TITLE: Syntax-based Constituent and Function Alignment for Parallel Treebanking
#+STARTUP: hidestars
...
This paper describes the development of an automatic phrase alignment
 ...
 constituents, or functional elements with dependency relations, may
 be linked. A set of principles for phrase alignment are formulated,
...
Lexical-Functional Grammar (LFG) is a grammatical framework where a
 ...
 between syntactic functions (predicate, subject, object, etc.), in
 addition to the grammatical features of these. The argument structure
 of predicates is embedded in the f-structure representation.

This work is part of the XPar-project, which involves developing an
 LFG-parsed parallel treebank for Dutch, Tigrinya, Georgian and
 ...
 (dependency elements). By utilising the information available in each
 monolingual LFG-parse of two parallel sentences, we are able to make
 precise and linguistically informative alignments on both the
 c-structure and f-structure level.

Although there exists many methods for automatic phrase alignment,
 e.g.  \cite{och2003scv}, most of these have been based on aligning
 any N-gram that is compatible with a word alignment, where none of
 these take into account syntactic features, and alignments may cross
...
Our method is instead based on the fact that similar grammatical
 ...
 with dependency relations: predicates, arguments and
 adjuncts[fn:12]. 
...
The current implementation is, as mentioned, a work in progress (in
 ...
 evaluation at this point. However, the results seem promising for the
 test sets we have tried.

A top-down method of alignment may be quite useful for
 lesser-resourced language pairs, if there exists LFG grammars for the
 languages. For a language pair such as Norwegian-Georgian or
 ...
 lessened. Given some manual intervention in selecting between
...
[fn:9] This is a basic tenet of the XPar-project, as well as of the
