...
In order to link two c-structure nodes, \cite[p.~77]{dyvik2009lmp}
 ...
 overtly expressed in Georgian[fn:5].  But by the criterion above we
 can still link the upper IP nodes, as they dominate the same sets of
 linked lexical nodes; the adjunct /gzaSi/ (``on the way'') is a
 translators addition only seen in the Georgian text, and remains
...
This example is worth a bit more attention.  For the main verbs of
both sentences to align in f-structure, all arguments would have to
find LPT-correspondences. 

The theme of /snakket/ (talked), /roboter/ (robots) is topicalised,
