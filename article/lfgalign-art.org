...
[fn:15] In particular: we don't yet align features like \SPEC{} and
 \POSS{} unless they are also in the arguments/adjuncts; also the
 current implementation uses LPT information to completely cut off
 possibilities---a ranking would be more robust if we have partial
 information. Additionally, the program expects sentence-aligned,
