...
\begin{frame}[plain]
  \title[Syntaktisk fraselenking]{
    \scalebox{1.2}{\centering\scriptsize{}\avmoptions{}
...
\begin{frame}[fragile]
  \begin{tikzpicture}[auto]
       ...
       \draw[-] ($(gibt.north)+(0.0,0.0)$) .. controls +(0,0) and +(0,0) .. ($(ein.north) +(0.0,0.0)$) ;
       \draw[-] ($(an.south)+(0.0,0.0)$) .. controls +(0,0) and +(0,0) .. ($(elephant.south) +(0.0,0.0)$) ;
       \draw[-] ($(ein.north)+(0.0,0.0)$) .. controls +(0,0) and +(0,0) .. ($(elefant.north) +(0.0,0.0)$) ;
       \draw[-] ($('s.south)+(0.0,0.0)$) .. controls +(0,0) and +(0,0) .. ($(an.south) +(0.0,0.0)$) ;
       \draw[-] ($('s.south)+(0.0,0.0)$) .. controls +(0,0) and +(0,0) .. ($(gibt.north) +(0.0,0.0)$) ;
       \draw[-] ($(elephant.south)+(0.0,0.0)$) .. controls +(0,0) and +(0,0) .. ($(elefant.north) +(0.0,0.0)$) ;
...
\begin{frame}
  ...
  {\footnotesize\setlength{\Exlabelsep}{1.0em} % was 1.3em
    \alignSubExtrue % wasn't
    \ex.
    \ag. \textbf{gza-\v{s}i} \textbf{robo\d{t}eb-ze} \textbf{la\d{p}ara\d{k}obdnen} \\
...
\begin{frame}
  \begin{itemize}
  \item Program/kjeldekode er tilgjengeleg frå
    \href{https://github.com/unhammer/lfgalign}{https://github.com/unhammer/lfgalign}
    under GNU GPL
  \item Desse lysarka kan distribuerast under lisensane
