...
\ex. PRED-elementa $p$ og $q$ kan berre lenkjast om dei er ytste
     f-strukturar eller er argument/adjunkt til lenkja f-strukturar.

# CURRENT TODO like omgivnader, top-down, treng ikkje vere likt inni, granularitet

*** SKRIV Kausativar og inkorporering
Eit meir komplisert problem er der éin f-struktur på kjeldespråket
korresponderer med to (eller fleire) på målspråket. Dette skjer
t.d. ved omsetjing av ein kausativ på georgisk til ein
/la/-konstruksjon på norsk, eller der eit verb på det eine språket har
inkorporert eit objekt som på det andre språket er uttrykt åleine.
...
{\avmoptions{}
\ex. \a. SUBJ $la-q$ OBJ \\
     $\begin{avm}\[pred & `{\bf{}la-q}<SUBJ, OBJ>' \]\end{avm}$
    $\\\\\leftrightarrow$\\
     \b. SUBJ lar OBJ $q$ \\
     $\begin{avm}\[pred & `{\bf{}la}<SUBJ, OBJ1, XCOMP:2>' \\
     xcomp & 2 \[pred & `{\bf{}q}<SUBJ1>'\]\]\end{avm}$
...
\fxnote[inline,nomargin]{TODO: dette blei litt implementasjonsinternt...spesielt det
med «motstridande informasjon»} Me kan sjølvsagt snu
