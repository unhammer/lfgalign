...
\citep{unhammer2009aaa} analyserte eg setningane frå den manuelt
...
\citep{butt2002pgp}, for å teste følgjande hypotese:
\begin{quote}
participants in a verbal situation are
expressed as arguments (rather than adjuncts) in the source language of a trans-
lation if and only if they are expressed as arguments (rather than adjuncts) in the
target language.
\end{quote}

For høgfrekvente innhaldsverb fann eg at berre 2 av 15
korresponderande verbtoken fekk LFG-analysar kor argument
korresponderte med adjunkt[fn:15], men datagrunnlaget her var svært
tynt.
...
argumentstruktur korresponderer med eit tematisk rollehierarki) er å
sjå på mange lenkjingar av same verbpar, og på den måten oppdage
moglege feil, men for slike enkelttilfelle vil krava i denne oppgåva
ikkje vere nok til å gi korrekt lenkjing.
