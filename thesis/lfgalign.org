...
Gitt ei viss f-struktursamanstilling, vil det berre vere ein mogleg
...
nemnt er det berre ein av mange moglege måtar å gjere det på, men eg
trur det tek innover seg dei viktigaste trekka.
...
\ex. La $m$ vere mengda av ein-til-ein LPT-korresponderande
     argument/adjunkt av to lenkja f-strukturar $F_s$ og $F_t$. La $n$
     vere dei elementa av $m$ som anten begge er adjunkt, eller begge
     er argument og har same posisjon i argumentstrukturane
     sine. /Følgjeskåren/ til $F_s$, $F_t$ er då $\frac{n}{m}$.
...
\ex. La $m$ vere mengda av ein-til-ein LPT-korresponderande
     argument/adjunkt av to lenkja f-strukturar $F_s$ og $F_t$. La $n$
     vere dei elementa av $m$ som anten ikkje har argument sjølve,
     eller som er lenkja på f-strukturnivå. /Djupnskåren/ til $F_s$,
     $F_t$ er då $\frac{n}{m}$.
...
\ref{SEC:rangering}. Implementasjonen av kriteria følgjer den
formaliseringa ganske eksakt. 
...
#+BEGIN_LaTeX
     ...
     \begin{algorithm}[]
      \caption{rank-f(seen, alignments)}
      \label{algo:rank-f}

      \uIf{f-link?(alignments)} {\Return (alignments, 1)}
      \uElse{
        best-rate $\gets$ 0 \;
 	best-branches $\gets \emptyset$ \;
        \ForAll{branches in alignment} {
        }
      }
      \end{algorithm}
#+END_LaTeX
...
desse «perfekte» lenkjer (så gode som det vil vere mogleg å få). 
...
ordposisjonane i to setningar. Viss $f_{1}^{J}=f_1,...,f_j,...,f_J$ er
orda i setninga på kjeldespråket og $e_{1}^{I}=e_1,...,e_i,...,e_I$ er
orda i setninga på målspråket[fn:27], er ei N-gramsamanstilling
$\mathcal{A}$ gitt ved $\mathcal{A}\subseteq \{(j,i):
j=1,...,J;i=1,...,I\}$. Eitt kjeldeord kan altså vere lenkja til eitt
eller fleire målord, og omvendt. 
...
Ei samanstilling er her altså ei mengd par av ordposisjonar. Det finst
mange ulike måtar å komme fram til samanstillinga på. Dei vanlege
metodane er basert på statistiske modellar, kor den beste
...
treningsmetode aukar den totale $p$ for heile korpuset (dvs. produktet
av $p$ for alle setningane). For å finne $p$ er det vanleg å nytte ein
skjult Markov-modell, dvs. at $p$ for heile setninga blir dekomponert
til $p$ for eitt enkelt kjeldeord, kor det siste er basert på $p$ av
orda som kjem før. I tillegg vil ein ofte vekte på sannsynet for ei
viss setningslengd gitt målsetninga (sidan lange setningar ofte blir
...
[fn:27] $e$ for engelsk og $f$ for fransk, sjølvsagt.
