...
beste evne i eit Lisp[fn:16]-program. Ei implementering gjer det au
svært synleg kor det finst manglar i eit formelt krav, eller om noko
ikkje er godt nok spesifisert.

Programmet =lfgalign= [fn:2] tek inn LFG-analysane av to setningar som
me veit er omsetjingar av kvarandre. LFG-analysane må vere
disambiguerte og i Prolog-formatet frå XLE[fn:5]. Programmet les inn
...
Hovudalgoritmen er vist i figur \ref{f-align}. Funksjonen =f-align=
returnerer ei liste med samanstillingar, kvar $alignment$ er ei liste
med par, lenkjer. =argalign= gir alle moglege samanstillingar av
/argumenta/ til $src$ og $trg$. 
...
$aligntable$. Dette føreset sjølvsagt at =f-align= $(s,t,LPT)$
...
$F_t$. \fixme{nemne dette «kravet» i forrige kapittel!}
...
\STATE \algorithmicfunction f-align($src$, $trg$, $LPT$) : alignments
  \FORALL{alignment in argalign($src$, $trg$, $LPTs$)}
     \FORALL{$src_a$, $trg_a$ in alignment}
       \IF{aligntable[$src$,$trg$] $eq$ $null$}
         \STATE aligntable[$src$,$trg$] $\gets$ f-align($src_a$, $trg_a$, $LPT$)
       \ENDIF
       \STATE $a \gets \{ (src, trg) , \}$
    \ENDFOR
  \ENDFOR
...
** gjer ikkje dette lenger					     :ROTETE:
=lfgalign= kan i tillegg ta inn ein representasjon av moglege
LPT-korrespondansar.
...
=permute= finn alle moglege permutasjonar av 1-1
LPT-korrespondansar. 
...
[fn:2] Tilgjengeleg frå http://github.com/unhammer/lfgalign under GNU
       General Public License.

[fn:5] Dokumentert på http://www2.parc.com/isl/groups/nltt/xle/doc/xle.html

[fn:16] Common Lisp ser ut til å vere standardspråket for høynivåkode
        til LFG-formål.
