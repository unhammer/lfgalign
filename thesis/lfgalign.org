...
- fraselenkjer vs frasesamanstilling :: Eg nyttar her termane
  ...
  /uttrykk/ (ord, setningar); lenkjing mellom dei siste altså er meir
  ateoretisk / datanært.
...
språket.»
** SKRIV Funksjonelle c-strukturnodar
\label{SEC:fnord}
...
Problemet med å nytte metoden frå del \ref{SEC:subnode} for i dette
...
via nokon f-strukturlenkjing).

Samtidig vil me ikkje at eit manglande funksjonsord på det eine
språket skal hindre lenkjing av komplementa, sidan det kan hende at
funksjonsordet ikkje er krevd på det språket (eventuelt kjem dette
fram som korrespondansar i f-strukturtrekk, kontroll av dette har eg
ikkje teke høgd for i denne oppgåva). Men viss begge språk har
funksjonsord, men funksjonsord som ikkje kan sjåast på som moglege
omsetjingar (t.d. /fordi/ og /whether/), bør me ikkje ein gong lenkje
komplementa[fn:10].
...
det gjekk bra» med /that/ i «he saw that she drew a picture»).
...
\b. Gitt ei mogleg lenkjing av FP og XP, der F er ein funksjonell
    ...
    den funksjonelle kategorien i c-strukturlenkjinga. F' er medlem av
    nodemengden som blir lenkja til XP.
...
ikkje er reflekter i CPnom til Ssub.
...
lenkjing av funksjonelle kategoriar.

I implementasjonen min har eg lagt mindre vekt lenkjing av
funksjonelle kategoriar sidan dei gjer f-strukturlenkjinga avhengig av
c-strukturforhold.
