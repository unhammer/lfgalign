...
\fxnote[inline,nomargin]{TODO: kva var meininga med dette avsnittet?}
...
** SKRIV Underordna c-strukturnodar
\label{SEC:subnode}
...
krav. \citet[s.~77]{dyvik2009lmp} definerer omgrepet /lenkja
leksikalske nodar/, $LL$, kor $LL(n)$ er mengda av nodar dominert av
$n$ som har ei ordlenkje; og krev at alle nodane i mengda $LL(n_s)$
(kor $n_s$ er ein lenkjingskandidat på kjeldesida) er lenkja til nodar
i $LL(n_t)$ (kor $n_t$ er ein lenkjingskandidat på målsida). Ulenkja
nodar står ikkje i vegen for lenkjing av $n_s$ og $n_t$.  For at me
skal unngå å lenkje I' og Vfin i figur \ref{fig:ikkjesub} må altså
/det/ og /seg/ vere ordlenkja, men ikkje til /gaiGo/.
\fxnote[inline,nomargin]{det går vel ikkje an?? skal \emph{det} vere
ordlenkja til ein \emph{del} av gaiGo? TOGROK}

Eg vel i staden å definere kravet i form av f-strukturlenkjer på
preterminale nodar, og dimed unngå krav på ordnivå her. 
...
\ex. \label{subnodekrav-infotap} For å lenkje dei underordna
     ...
     som er dominert av $n_s$. Ein ulenkja node $n'_s$ har ei «tom»
     f-strukturlenkje frå $\phi(n'_s)$ til $\emptyset$. Nodane $n_s$ og $n_t$
     kan då lenkjast viss
...
\c.  og $L_c(m_s)-L_c(n_s)=L_c(m_t)-L-c(n_t)$.
...
Figur \ref{fig:enkelsub2} illusterer dette kravet.

# TODO: avsnittet nedanfor til figuren og avsnittet over for å
# forklare \Last[c] og omgrepet informasjonstap
