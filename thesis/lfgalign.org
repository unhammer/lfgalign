...
* Bakgrunn og omgrepsavklaring					     :ROTETE:
  \label{SEC:bakgrunn}
...
I det minste vil me altså krevje følgjande av lenkjer på c-strukturnivå:
...
NP-subjektet på den norske sida, er ikkje lenkja med
noko i det georgiske treet. Den informasjonen (her reint
syntaktisk) som ordet /det/ tilfører IP, ligg under I' på georgisk; og
ved I-nodane manglar det norske treet i tillegg den informasjonen som
/seg/ tilfører. 
...
\end{figure}
#+END_LaTeX
\fxnote{TODO fotnote om at eg har forenkla trea litt, ikkje PRONP og
PRONrfl og Vfin og slikt}
...
Det finst (iallfall) to alternativ. Me kan seie at I- og I'-nodane
ikkje skal lenkjast, sidan /det/ og /seg/ er lenkja på f-strukturnivå
(til subjekt og objekt av gaiGo), då tolker me det slik at I' og IP
...
kan formalisere dette kravet i del \ref{SEC:infotap}.
...
domena til /gaiGo-åpnet/ i figur \ref{fig:gaiGo}, sidan
$LL(IP_s)=LL(I'_s)=LL(I_s)=\{(\text{gaiGo},\text{åpnet})\}=LL(IP_t)=LL(I'_t)=LL(I_t)$,
kor georgisk er kjeldespråket og har subskript $_s$, og norsk er
målspråket med subskript $_t$, og /det/ og /seg/ er ulenkja på både
c-strukturnivå og ordnivå[fn:26].

\fxnote{TODO: diskutere litt meir forskjellane på desse alternativa}

*** Informasjonstap som lenkjingskriterium
\label{SEC:infotap}

Sidan det er mogleg å ønskje seg å ikkje lenkje I'- og I-nodane i
\ref{fig:gaiGo}, gir eg her ei alternativ løysing. 
...
\end{scope}
  \draw[-,very thick] (I's)..controls +(north east:1.5) and +(north west:1.5) .. (I't) ;
  \draw[-] (OBJs)..controls +(north east:2) and +(west:1) .. (OBJt) ;
\end{tikzpicture}
   \caption{Enkel lenkjing av c-strukturnodar mellom norsk og
   nederlandsk; IP til IP, I' til I' og I til I.}
...
Me treng altså eit litt meir nyansert krav. Som nemnte i fotnote
...
\ex. \label{krav:c-infotap} For å lenkje c-strukturnodane $n_s$ og $n_t$:\\
     La $L_c(n_s)$ vere alle
     f-strukturlenkjer frå $\phi(n'_s)$ for alle preterminale $n'_s$
     som er dominert av $n_s$. Nodar utan f-strukturlenkjer er ikkje med i $L_c$.
     Nodane $n_s$ og $n_t$ kan då lenkjast viss $L_c(m_s)-L_c(n_s)=L_c(m_t)-L_c(n_t)$.
...
toppnodar, lenkja ved krav \ref{krav:f-links-domain}. I'-nodane er i lenkja
...
% \draw[-] (gaiGo)..controls +(south:1) and +(south:1) .. (åpnet) ;
 \draw[-] (NPk)..controls +(north east:1) and +(north west:1) .. (NPb) ;
...
min har eg valt å følgje krav \ref{krav:c-infotap} for å unngå å
...
krav \ref{krav:subnode-f-lenkja}
vere oppfylt kva gjeld CP-komplementa -- lenkjinga går ikkje ut over
dei funksjonelle domena, medan krav~\ref{krav:c-infotap} er
...
* Implementasjonen av =lfgalign=				     :ROTETE:
...
Programmet =lfgalign= følgjer krav \ref{krav:c-infotap} og
...
[fn:26] \label{fn:LL-ordlenkje} Merk at orda /det/ og /seg/ måtte definerast som ulenkja for
        ...
        \ref{krav:c-infotap} i del \ref{SEC:infotap} gir.
