...
I \Last har /la-fryse/ to argument, som ved krav \ref{PRED-krav} begge
...
\ref{PRED-omgivnad-krav} og top-down-metoden til f-align hindra oss i
...
Det viktige forholdet på c-strukturnivå er /dominans/; ein c-struktur
gir ein hierarkisk inndeling av frasestrukturen i setninga, der ein
node på høgare nivå /dominerer/ mengder av nodar på lågare nivå. 

#+BEGIN_LaTeX
\begin{figure}[htp]
\fxnote{todo: korrekt f-domene-annotasjon}
\centering
  \begin{tikzpicture}
  \Tree  [.\node(IPs){IP};  [.\node(SUBJs){NP \^{ }SUBJ=v}; \edge[roof]; {det} ]
                            [.\node(I's){I'};
				    [.\node(Is){I}; {regner} ]
				    [.\node(OBJs){NP \^{ }OBJ=v}; \edge[roof]; {kjøttbollar} ] ] ]
      \begin{scope}[shift={(2in,0in)}]
  \Tree  [.\node(IPt){IP};  [.\node(SUBJt){NP \^{ }SUBJ=v}; \edge[roof]; {het} ] 
                            [.\node(I't){I'}; 
				    [.\node(It){I}; {regent} ]
				    [.\node(OBJt){NP \^{ }OBJ=v}; \edge[roof]; {gehaktballen} ] ]   ]
\end{scope}
%\draw[-] (IPs)..controls +(north east:1) and +(north:1) .. (IPt) ;
%\draw[dashed,-] (I's)..controls +(south:1) and +(south:1) .. node[above,sloped]{?} (I't) ;
%\draw[dashed,-] (SUBJs)..controls +(south:1) and +(south:1) .. node[below,sloped]{manglar} (SUBJt) ;
%\draw[dashed,-] (OBJs)..controls +(south:1) and +(south:1) .. node[below,sloped]{manglar} (OBJt) ;
...
I figur \ref{fig:enkel-c-lenkje} er dei funksjonelle domena til /regne/regent/ 
lenkja, og det same med /det/het/ og /kjøttbollar/gehaktballen/. 
...
kjeldesida dominerer I' subjektet, som er lenkja til subjektet på
målsida, men på målsida står ikkje subjektet under I', og omvendt for
...
i figur \ref{fig:ikkje-c-lenkje}, medan i \ref{fig:enkel-c-lenkje}
...
\centering
  \begin{tikzpicture}
  \Tree  [.\node(IPs){IP};  [.\node(OBJs){NP \^{ }OBJ=v}; \edge[roof]; {kjøttboller} ]
                            [.\node(I's){I'};
				    [.\node(Is){I}; {regner} ]
				    [.\node(SUBJs){NP \^{ }SUBJ=v}; \edge[roof]; {det} ]
				     ] ]
...
\draw[-] (OBJs)..controls +(south east:1) and +(south west:1) ..  (OBJt) ;
...
pro-drop-språk, ville me fått same situasjon; I'-nodane i figur
\ref{fig:ikkje-c-lenkje-pro-drop} kan ikkje lenkjast sidan I' på
islandsk dominerer objektet, medan I' på norsk ikkje gjer dette, og
objekta er lenkja med kvarandre. Ei lenkje mellom desse I'-nodane
ville sagt at dei dominerer korresponderande materiale, men det gjer
dei ikkje her. 
...
\centering
  ...
  \Tree  [.\node(IPt){IP};  
                            [.\node(I't){I'}; 
				    [.\node(It){I}; {rignir} ]
				    [.\node(OBJt){NP \^{ }OBJ=v}; \edge[roof]; {kjötbollum} ] ]   ]
\end{scope}
\draw[-] (IPs)..controls +(north east:1) and +(north:1) .. (IPt) ;
\draw[dashed,-] (I's)..controls +(north:1) and +(north:1) .. node[midway,sloped]{$\times$} (I't) ;
\draw[-] (OBJs)..controls +(south east:1) and +(south west:1) ..  (OBJt) ;
...
c-strukturnodar som går på tvers av f-strukturlenkjer til å ta innover
seg dominans: me vil ikkje lenkje c-strukturnodar viss det dei
dominerer går på tvers av f-strukturlenkjer.

\fxnote[inline,nomargin]{CURRENT TODO (kople avsnittet over til det
med dominans nemndt tidlegare)}
...
I enkle tilfelle vil nodane dominert av toppnoden XP i eit funksjonelt
domene inkludere alt som står under $\phi(XP)$ i f-strukturen. Men i
ei setning som «Kven sa du at du likte?» vil objektet til /like/
stå utanfor det funksjonelle domenet til /like/, om me skal lenkje
dette til «You said you liked who?»
...
Programmet =lfgalign= følgjer krav \ref{subnodekrav-infotap} og
...
## TODO CURRENT!!
# Me hentar så ut alle dei lenkjemengdene som er i begge tabellane
# (dvs. snittet av oppslagsnøklene til tabellen); nodane som er lagra
# med mengd med f-strukturlenkjer skal lenkjast på c-strukturnivå. Alle
# desse mange-til-mange-lenkjene blir til slutt returnert av =c-align=.
