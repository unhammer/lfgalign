...
=argalign= (figur \ref{argalign}) gir alle moglege samanstillingar av
...
#+BEGIN_LaTeX
  \SetAlgorithmName{Algoritme}{algoritme}{liste over algoritmar}
  \SetAlgoFuncName{Funksjon}{fn}
    ...
    \ForAll{argperm in argalign($F_s$, $F_t$, $LPTs$)} {
       \ForAll{$A_s$, $A_t$ in argperm} {
         \If{aligntable[$F_s$,$F_t$] $=$ $null$} {
    ...
    adjuncts($F_s$), arguments($F_t$), adjuncts($F_t$), $LPT$)}
    \BlankLine
    ...
    \lIf{$args_s$} {
        ...
        \ElseIf{$args_t$} {
          \If{$adjs_s$}{
            \ForAll{$t$ in $trg$} {
              $s \in adjs_s$\;
              \ForAll{$perm$ in argalign-p($args_s$, $adjs_s-s$, $args_t-t$,
                $adjs_t$)}{
                $foo \gets \{(s,t)\} \bigcup perm$
              }
            }
            mapalign\;
          }\lElse{
