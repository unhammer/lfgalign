...
** SKRIV med argumenter meiner eg her arg + andre subkat.ledd
  

** SKRIV men kanskje me burde ha fordi/whether for å finne dei
** SKRIV bleu + koehn del 3.2 er ikkje klart
  PCFG-parsar
  høgare bleu = betre
** TODO finn alle ufragmenterte RIA-setningar
...
=RIA=-metoden[fn:44] i \citet{graham2009osr,graham2009fts}. Eg prøvde
både utan nokon informasjon i LPT-tabellen, og med ein ganske stor
LPT-tabell med alle oppslaga frå Ding-ordboka[fn:54].
...
#+LABEL: tbl:RIA
#+CAPTION: Overlapp mellom \texttt{RIA} og \texttt{lfgalign}, og mellom \texttt{RIA} og ein tilfeldige baseline. Det fulle testsettet har totalt 40343 lenkbare \PRED{}-element på kjeldesida; 337 når alle setningspar med fragmentariske analysar er fjerna.
#+ATTR_LaTeX: align=lcrrrrr
| metode                     | maks.frag. | snitt | union | lenkjer | \texttt{RIA}-lenkjer | snitt / \texttt{RIA} |
|----------------------------+------------+-------+-------+---------+----------------------+----------------------|
| \texttt{tilfeldig}         |          0 |    43 |   526 |     307 |                  262 | 16,41 %              |
| \texttt{lfgalign} u/ordbok |          0 |   142 |   386 |     272 |                  262 | 54,20 %              |
| \texttt{lfgalign} m/ordbok |          0 |    51 |   302 |      91 |                  262 | 19,47 %              |
|----------------------------+------------+-------+-------+---------+----------------------+----------------------|
| \texttt{tilfeldig}         |          1 |   292 |  4151 |    2426 |                 2017 | 14,48 %              |
| \texttt{lfgalign} u/ordbok |          1 |  1066 |  3029 |    2127 |                 2017 | 52,85 %              |
| \texttt{lfgalign} m/ordbok |          1 |   409 |  2327 |     725 |                 2017 | 20,28 %              |
|----------------------------+------------+-------+-------+---------+----------------------+----------------------|
| \texttt{tilfeldig}         |          2 |   682 | 12153 |    7005 |                 5830 | 11,70 %              |
| \texttt{lfgalign} u/ordbok |          2 |  2586 |  9028 |    5890 |                 5830 | 44,36 %              |
| \texttt{lfgalign} m/ordbok |          2 |   985 |  6684 |    1856 |                 5830 | 16,90 %              |
|----------------------------+------------+-------+-------+---------+----------------------+----------------------|
| \texttt{tilfeldig}         |   $\infty$ |  2544 | 60544 |   35069 |                28019 | 9,08 %               |
| \texttt{lfgalign} u/ordbok |   $\infty$ |  7296 | 44587 |   24161 |                28019 | 26,04 %              |
| \texttt{lfgalign} m/ordbok |   $\infty$ |  3382 | 31970 |    7375 |                28019 | 12,07 %              |
#+TBLFM: $7='(replace-regexp-in-string "[.]" "," (format "%.2f %%" (* 100 (/ $3 (* 1.0 $6)))));N

### Code to re-run analyses, one line per row:
...
# (ev-ria (ria-analyses nil nil) #'f-align        #'rank        (ding-LPT))
 ...
 setningspar, mellom =RIA= og dei tri testmetodane (=lfgalign= med og
 utan informasjon i LPT-tabellen, og ein tilfeldig baseline). Ved å
 dele dette talet på kor mange lenkjer =RIA= hadde, får me ein enkel
 ...
 =RIA= og =lfgalign= er svært lågt på dette materialet[fn:46], men
 ...
 dei har ulike lenkjer, er det =lfgalign= (av og til begge) som tek
 ...
 med, får =lfgalign= berre ein fjerdedel så mange lenkjer som =RIA=.
 Det er kanskje ein del feil blant lenkjene i =RIA=-testsettet; men
 sjølv om halvparten av lenkjene til =RIA= var feil, burde me sjå
 høgare snitt mellom =lfgalign= og =RIA= viss ordboka førte til betre
 lenkjer. Det viser seg allereie med første ord i testsettet kvifor
 dette skjer: ordboka omset engelsk /adoption/ med tysk /Adoption/ (og
 gir ingen andre omsetjingar), medan /adoption/ i denne setninga er
 nytta i ei anna tyding og blir omsett til /Genehmigung/ (som har ti
 ...
 så låg dekning (12 % viss =RIA= har rett, 8 % viss me ser på alle
 ...
 store delar av analysane får =lfgalign= ingenting ut av f-strukturen.
 Sidan programmet er meint for trebankbygging med høg presisjon treng
 det ikkje vere negativt at dekninga er /litt/ lågare; i tillegg vil
 me kanskje forvente meir «komplette» analysar enn dei som finst i
 dette testsettet. Likevel bør det vere mogleg å gjere noko for å
 betre på yteevna når analysane er fragmentariske; eg kjem tilbake til
 dette nedanfor.
 ...
 (med ordboka tok det under eitt minutt, her blir altså /svært mange/
...
[fn:46] Testen utan ordbok blei køyrt på revisjon
        =fd14d902262a9274b42131cf83ade74d51e23863= av =lfgalign=,
        testen med ordbok blei køyrt på revisjon
        =a92646e4673dc839488fe4f50f739c8d5e45dc2e=.  Sjå funksjonen
        =ev-ria=.
