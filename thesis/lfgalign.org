...
** N-grambaserte metodar
 ...
 setningar i eit korpus, sannsynlegvis er omsetjingar.  Dei vanlegaste
 metodane er basert på sannsynsmodellar[fn:36], kor den beste
 ...
 treningsmetode aukar den totale $p$ for heile korpuset (dvs. summen
 av $p$ for alle setningane). For å finne $p$ er det vanleg å nytte
 ein skjult Markov-modell, dvs. at $p$ for heile setninga blir
 dekomponert til $p$ for eitt enkelt kjeldeord, kor p av enkeltord er
 basert på $p$ av orda som kjem før. I tillegg vil ein ofte vekte på
 sannsynet for ei viss setningslengd gitt målsetninga (sidan lange
 setningar ofte blir omsett til like lange setningar, modulo
